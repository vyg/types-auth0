...
export interface AuthParamOptions {
  domain?: string;
  clientId?: string;
  clientSecret?: string;
  audience?: string;
...
interface SignUpUser<A=AppMetadata, U=UserMetadata> extends User {
 _id: string;
}
...
export class DatabaseAuthenticator<A = AppMetadata, U = UserMetadata> {
    ...
    signUp(data: SignUpUserData): Promise<SignUpUser<A, U>>;
    signIn(data: SignUpUserData, cb: (err: Error, data: SignUpUser) => void): void;
}
